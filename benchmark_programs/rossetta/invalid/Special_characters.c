...
'''String and character literals''' are like C++ (or rather the other way around!), and even the meaning and usage of the <tt>#</tt> character is the same./* <stdin>:4:52: warning: missing terminating ' character [-Winvalid-pp-token]
As in C++, <tt>?</tt>, <tt>#</tt>, <tt>\</tt>, <tt>'</tt> and <tt>"</tt> have special meaning (altogether with <tt>{</tt> and <tt>}</tt>). Also trigraphs work (they are an "old" way to avoid the "old" difficulties of finding characters like { } etc. on some keyboards).
                                                   ^
<stdin>:75:59: warning: empty character constant [-Winvalid-pp-token]
C99 standard (but not previous standards) recognizes also '''universal character names''', like C++.
                                                          ^
<stdin>:75:88: warning: empty character constant [-Winvalid-pp-token]
C99 standard (but not previous standards) recognizes also '''universal character names''', like C++.
                                                                                       ^
<stdin>:77:1: warning: empty character constant [-Winvalid-pp-token]
...
^
<stdin>:77:34: warning: empty character constant [-Winvalid-pp-token]
'''String and character literals''' are like C++ (or rather the other way around!), and even the meaning and usage of the <tt>#</tt> character is the same.
                                 ^
