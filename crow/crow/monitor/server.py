...
@app.route('/programs/<page>', methods=['GET'])
def programs(page):
    conn = get_connection()
    return jsonify(get_all_programs(conn, page=int(page)))

# Page zero
@app.route('/programs', methods=['GET'])
def programs_first_page():
    conn = get_connection()
    return jsonify(get_all_programs(conn))


@app.route('/bitcodes/unique/<programname>')
def unique_bitcodes(programname):
    conn = get_connection()
    return jsonify(get_unique_bitcodes(conn, programname))

 
@app.route('/wasm/unique/<programname>')
def unique_wasms(programname):
    conn = get_connection()
    return jsonify(get_unique_wasm(conn, programname))


@app.route('/eq/bc/<bitcode_key>')
def get_equal_variants_for_bc(bitcode_key):
    conn = get_connection()
    return jsonify(get_eq_variants_for_bc(conn, bitcode_key))   

@app.route('/eq/wasm/<bitcode_key>')
def get_equal_variants_for_wasm(bitcode_key):
    conn = get_connection()
    return jsonify(get_eq_variants_for_wasm(conn, bitcode_key))   
