...
from crow.monitor.minpeewee.readdb import connect_db, SubscriberStats, Module, Function, Variant, BCFile, WasmFile, WatFile
import os, sys
    ...
    try:
        if tpe == CROW_HEARTBEAT_GENERATED:
            ...
            try:
                ...
                stat.cpu_usage = data['cpu_usage']
                stat.memory_usage = json.dumps(data['memory_usage'])
                stat.save()
                ...
                stats = SubscriberStats.create(
                    ...
                    cpu_usage=data['cpu_usage'], memory_usage = json.dumps(data['memory_usage'])
                    )
