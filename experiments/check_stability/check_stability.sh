...
if [[ $(find "$BC_FOLDER" -name "*.bc" -depth 1 | wc -l) != $(find "$WASM_FOLDER" -name "*.wasm" -depth 1 | wc -l) ]]
then
    echo "WARNING It seems that the wasm generation is not complete"
    echo -n "bc"
    find "$BC_FOLDER" -name "*.bc" -depth 1 | wc -l
    echo -n "wasm"
    find "$WASM_FOLDER" -name "*.wasm" -depth 1 | wc -l

    if [[ $STOPAT_WARNING == "True" ]]
    then
        exit 1
    fi
fi
...
function compare() {
    ...
    VARIANT1_NAME=$(basename "$f" | grep -Eo '_\[.*]')
    VARIANT2_NAME=$(basename "$f2" | grep -Eo '_\[.*]')

    VARIANT1=$(echo $VARIANT1_NAME | sed s/.$//)
    VARIANT1=$(echo $VARIANT1 | sed s/^.//)
    VARIANT1=$(echo $VARIANT1 | sed s/^.//)


    VARIANT2=$(echo $VARIANT2_NAME | sed s/.$//)
    VARIANT2=$(echo $VARIANT2 | sed s/^.//)
    VARIANT2=$(echo $VARIANT2 | sed s/^.//)


    R1=$(get_replacement_SouperIR "8789" "$VARIANT1")
    R2=$(get_replacement_SouperIR "8789" "$VARIANT2")
    #python3 get_keys.py <<< "$R"
    V1=$(python3 get_values.py <<< "$R1" || echo "NOT FOUND")
    V2=$(python3 get_values.py <<< "$R2" || echo "NOT FOUND")
    ...
    then
        ...
        notify "$PROGRAM_NAME: $VARIANT1 = $VARIANT2"
        ...
        COUNT_DIFF_BC=$(bc <<< "$COUNT_DIFF_BC + 1")

        WASM1=$PROGRAM_NAME$VARIANT1_NAME.bc.wasm
        WASM2=$PROGRAM_NAME$VARIANT2_NAME.bc.wasm

        if test -f "$WASM_FOLDER/$WASM1"; then

            if test -f "$WASM_FOLDER/$WASM2"; then
                

                MD5_WASM1=$(md5sum "$WASM_FOLDER/$WASM1" | awk '{print $1}')
                MD5_WASM2=$(md5sum "$WASM_FOLDER/$WASM2" | awk '{print $1}')
                ...
                then
                    ...
                    echo "bc($VARIANT1_NAME,$VARIANT2_NAME) = 1 but wasm($VARIANT1_NAME,$VARIANT2_NAME) = 0"
                    notify "$PROGRAM_NAME  bc($VARIANT1_NAME,$VARIANT2_NAME) = 1 but wasm($VARIANT1_NAME,$VARIANT2_NAME) = 0"

                    COUNT_CASE1=$(bc <<< "$COUNT_CASE1 + 1")
                    set_case1 "8789" $PROGRAM_NAME $VARIANT1 $VARIANT2

                    # Copy the variants to the out folder
                    cp $WASM_FOLDER/$WASM1 out/$PROGRAM_NAME/wasm
                    cp $WASM_FOLDER/$WASM2 out/$PROGRAM_NAME/wasm

                    # Get the textual representation to check after
                    wasm2wat $WASM_FOLDER/$WASM1 -o out/$PROGRAM_NAME/wasm/$WASM1.wat
                    wasm2wat $WASM_FOLDER/$WASM2 -o out/$PROGRAM_NAME/wasm/$WASM2.wat
                    ...
                    do
                        ...
                        n1=$PROGRAM_NAME$VARIANT1_NAME.bc.wasm.$arch.native
                        n2=$PROGRAM_NAME$VARIANT2_NAME.bc.wasm.$arch.native

                        ERR1=$($WASMTIME wasm2obj --target=$arch $WASM_FOLDER/$WASM1 out/$PROGRAM_NAME/native/$n1 2>&1) || ERROR=1

                        ERR2=$($WASMTIME wasm2obj --target=$arch $WASM_FOLDER/$WASM2 out/$PROGRAM_NAME/native/$n2 2<&1) || ERROR=1
                        ...
                        if [ $MD5_NATIVE1 != $MD5_NATIVE2 ]
                            then
                                ...
                                if [[ $SAVE_DIFF == "True" ]]
                                then
                                    cp $WASM_FOLDER/$WASM1 out/$PROGRAM_NAME/diff_wasm &
                                    cp $WASM_FOLDER/$WASM2 out/$PROGRAM_NAME/diff_wasm &
                                ...
                                echo $WASM_FOLDER/$WASM1 $WASM_FOLDER/$WASM2
                                wasm2wat $WASM_FOLDER/$WASM1 -o out/$PROGRAM_NAME/wasm/$DIFF_COUNT/$WASM1.wat

                                wasm2wat $WASM_FOLDER/$WASM2 -o out/$PROGRAM_NAME/wasm/$DIFF_COUNT/$WASM2.wat

                                DIFF_COUNT=$(bc <<< "$DIFF_COUNT + 1")
                                D=$(diff -y --suppress-common-lines out/$PROGRAM_NAME/wasm/$WASM1.wat out/$PROGRAM_NAME/wasm/$WASM2.wat)

                                notify "$PROGRAM_NAME bc($VARIANT1_NAME,$VARIANT2_NAME) = 1 and wasm($VARIANT1_NAME,$VARIANT2_NAME) = 1 but $arch($VARIANT1_NAME,$VARIANT2_NAME) = 0"

                                echo "$D" > D.txt
                                notify_file "./D.txt"

                                set_case2 "8789" $PROGRAM_NAME $VARIANT1 $VARIANT2 "$(cat out/$PROGRAM_NAME/wasm/$WASM1.wat)" "$(cat out/$PROGRAM_NAME/wasm/$WASM2.wat)" "$V1" "$V2"


                                COUNT_CASE2=$(bc <<< "$COUNT_CASE2 + 1")
...
echo "DIFF  BC $COUNT_DIFF_BC, EQ BC $COUNT_EQ_BC"
