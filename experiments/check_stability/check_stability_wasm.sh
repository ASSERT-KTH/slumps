...
function compare() {
    ...
    K1=$(python3 get_keys.py <<< "$R1" || echo "NOT FOUND")
    K2=$(python3 get_keys.py <<< "$R2" || echo "NOT FOUND")

    #WASM1=$PROGRAM_NAME$VARIANT1_NAME.bc.wasm
    #WASM2=$PROGRAM_NAME$VARIANT2_NAME.bc.wasm
    WASM1=$PROGRAM_NAME$VARIANT1_NAME.wasm
    WASM2=$PROGRAM_NAME$VARIANT2_NAME.wasm
                ...
                do
                    ...
                    if [ $MD5_NATIVE1 != $MD5_NATIVE2 ]
                        then
                            ...
                            echo "$K1" > out/$PROGRAM_NAME/wasm/$DIFF_COUNT/k1.O.json
                            echo "$K2" > out/$PROGRAM_NAME/wasm/$DIFF_COUNT/k2.O.json
                            echo "$V1" > out/$PROGRAM_NAME/wasm/$DIFF_COUNT/$WASM1.R.json
                            echo "$V2" > out/$PROGRAM_NAME/wasm/$DIFF_COUNT/$WASM2.R.json
